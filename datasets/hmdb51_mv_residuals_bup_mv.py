...
def make_dataset(opt, annotation_path, subset, n_samples_for_each_video,
    ...
    for i in range(len(video_names)):
        video_path = os.path.join(opt.video_path,video_names[i]) + ".avi"
        if i % 1000 == 0:
...
class HMDB51(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.spatial_transform is not None:
            ...
            #print("Before:",len(clip))
            if len(clip) < 12:
                delta = 12 - len(clip)
                dup = clip[-1]
                for k in range(delta):
                    clip.append(dup)
               # print("After:",len(clip))
        ...
        if self.opt.mv:
            ...
            if self.opt.add:
                ...
                for mat in clip:
                    if l != 0:
                        mv_comb = (mvclip[0,l,:,:]+mvclip[1,l,:,:])/2
                        mv_comb =torch.unsqueeze(mv_comb,0)
                        mv_frame = torch.cat((mv_comb,mv_comb,mv_comb),0)                    
                        residual_frame = clip[:,l,:,:]
                        clip [:,l,:,:] = (residual_frame + iframe)*mv_frame/2
                    l += 1
