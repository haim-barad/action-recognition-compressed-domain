...
def make_dataset(opt,root_path, annotation_path, subset, n_samples_for_each_video,
    ...
    #print("data:",data)
    labels = torch.load("kinetics_labels.json")
    video_names, annotations = get_video_names_and_annotations(data, subset)
    
    class_to_idx = get_class_labels(labels)
    ...
    for i in range(len(video_names)):
        ...
        #video_names[i] = video_names[i].split('_')[0]
        #print(video_names[i])
...
class Kinetics(data.Dataset):
    ...
    def __init__(self,opt,
        ...
        self.data, self.class_names = make_dataset(opt,
            ...
            frames_sequence)
        with open("kinetics_dataset.txt",'w') as f:
    ...
    def __getitem__(self, index):
        ...
        if self.opt.mult_loss:
            return clip,clip, target
        else:            
            return clip, target
